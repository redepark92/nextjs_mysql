...
//async function getMemoDetail(pri_no: string): Promise<MemoDetail[]> {
function getMemoDetail(pri_no: string) {
  //const [memo, setMemo] = useState<MemoDetail[]>([]);
  ...
  try {
    useEffect(() => {    
      ...
      const fetchData = async () => {
        ...
        //const baseurl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/memoContent`;
        const params = {
          pri_no: pri_no,
        }
    ...
    //의존성 배열을 빈 배열([])로 설정하면, 컴포넌트가 처음 마운트될 때만 실행됩니다. [pri_no] 대신에 [] 만 해도 된다. 
      fetchData();
    }, [pri_no]);
    ...
    console.log('Memo 가져오기 실패:');
    console.error('Memo 가져오기 실패:', error);
    // 주의 return []; 을 유지하면 안된다. try 에서 넘기는 값과 타입을 맞추어야 한다. 
    return ;
  }
...
export default function MemoDetailPage() {
  ...
  const memoDetail = getMemoDetail(id);
...
//  const history = await getMemoHistory(pri_no);
