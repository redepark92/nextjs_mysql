...
//  console.log({"nowPage is" : nowPage, "limit is" : limit, "lastPages is":lastPages, "firsNum is" : firstNum,  "lastNum is" : lastNum  });
  ...
  return (
    <div className="mt-4 flex justify-center space-x-2">
      <div className={`flex size-10 items-center justify-center rounded-full bg-gray-200 p-2 ${firstNum < limit ? "pointer-events-none " : ""}`} >
        <Link  href={`/memopage?page=${firstNum-limit}&${PageUrlString}`}> <FiChevronLeft /> </Link>
      </div>
      
      {Array(pagelimit).fill(0).map((_, i) =>{
        return (
          <div key={i} className={`flex size-10 items-center justify-center rounded-full 
            ${nowPage === firstNum + i ? 'bg-blue-300 text-white' : 'bg-gray-200'}`}>
            <Link href={`/memopage?page=${firstNum+i}&${PageUrlString}`}> {firstNum+i} </Link>
          </div>
        )
      })}

      <div className={`flex size-10 items-center justify-center rounded-full bg-gray-200 p-2 ${firstNum+limit > lastPages ? "pointer-events-none" : ""}`} >
        <Link href={`/memopage?page=${firstNum+limit}&${PageUrlString}`}> <FiChevronRight /> </Link>
      </div>
